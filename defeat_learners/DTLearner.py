...
class DTLearner(object):
    ...
    def build_tree(self, data):
        ...
        if (data.shape[0] <= self.leaf_size):
            ...
            best_feature_idx = self.correlation(data)
            split_val = np.median(data[:, best_feature_idx])

            maximum = max(data[:, best_feature_idx])
            if (maximum == split_val):
                return np.array([[-404, np.mean(data[:, -1]), -1, -1]])

            left = self.build_tree(data[data[:, best_feature_idx] <= split_val])
            right = self.build_tree(data[data[:, best_feature_idx] > split_val])

            root = np.array([[best_feature_idx, split_val, 1, left.shape[0] + 1]])
